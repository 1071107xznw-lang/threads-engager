...
> 而它能用你的身分發文。如果這台電腦跟客用 Wi-Fi 同一個網路（店面很常見），
> 密碼就是唯一防線。只在自己電腦上用的話，在 `.env` 設 `HOST=127.0.0.1` 直接把區網關掉。
...
**⏰ 建議發文時間**：產完稿會自動幫每則填一個時間，卡片上標「⏰ 建議（可改）」。

> 🔴 **只填時間，不代替你核准。** 狀態維持「待審核」，時間到了也**不會**被發出去 ——
> `publishDue` 只撈 `status='approved'`。你按「排程」才會進發布佇列。
> 這是合規紅線（規則 2），有專門的測試守著。

時段從**你自己的**貼文數據算：把自家貼文按小時分桶、比較平均瀏覽，好的時段先用。
自家貼文少於 `bestTimeMinSamples`（預設 10）則時回落預設值，
並在 log／UI 標明「這是預設值不是實測」—— 不會假裝算過了。

| 設定 | 預設 | 管什麼 |
|---|---|---|
| `dailyPublishCap` | 15 | 每個本地日最多排幾則 |
| `minGapMinutes` | 55 | 兩則之間至少隔多久 |
| `activeHours` | `{start:9,end:25}` | 只排在這個窗內（25 = 隔日 01:00） |
| `autoSchedule` | `true` | 設 `false` 就完全不自動填時間 |

> **為什麼是「間隔」而不是「少發一點」**：官方 API 硬上限是 **250 則/24 小時**，
> 一天 15 則差得很遠。真正的代價是同帳號連續刷版時**自己的貼文互相稀釋推薦池**，
> 所以防線是時間分散。發一兩週後看「平均每則瀏覽」有沒有掉 ——
> 掉了就把 `dailyPublishCap` 調到 5～8。

**發布額度守門**：發之前先問官方 `threads_publishing_limit` 還剩多少。
用完就在**建立容器之前**擋下來（建了才失敗會留下無主的 container）；
剩 10 則以內會警告。狀態列顯示「今日已發 N / 250」。
額度查詢本身失敗**不擋發布** —— 那是保護機制，不是發文的前提。
...
3. **⚖️ 法規紅線**（[`src/compliance.mjs`](src/compliance.mjs)）：酒在台灣屬「食品」，受
   《食品安全衛生管理法》第 28 條管——§28-2 醫療效能宣稱**罰 60 萬起**、§28-1 誇張不實罰 4 萬起。
   主管機關明文：依「整體廣告表現」判斷，**即使以隱喻、暗示、諧音呈現**也算。
   所以「解酒／保肝／助眠／瘦身／不醉不歸／保證」這類說法在三條產出路徑
   （產稿紅隊、✨優化、💬留言區）都被同一份規則擋掉，**連當玩笑帶到都不行**——
   截圖傳出去時，你括號裡的否認不會跟著走。
   另有高精準關鍵字掃描：dashboard 列表時掃一次，命中就標 `⚖️ 法規風險` 並附條款與罰則。
   **警示不阻擋**，仍由你決定。（刻意不誤判「醒酒／醒酒器」「療癒」這類正常用語。）

4. **🛡 紅隊審稿**：每則草稿再讓 AI 扮演「最愛抓語病的知識型網友」挑一遍，
...
| | 誰在管 | 能不能調 |
|---|---|---|
| ⚖️ 法規紅線 | 食安法（療效宣稱、鼓勵過量、酒駕、未成年） | ❌ 任何尺度都不能碰 |
| 😈 幽默尺度 | 你的品牌個性 | ✅ 你自己決定 |

`config/brand.json` 的 `humor`：

| 值 | 尺度 |
|---|---|
| `mild` | 溫和。友善、自嘲、生活感小幽默 |
| `spicy`（**預設**） | 有梗。吐槽、耍賤、雙關與成人向暗示。「生蠔日帶爸爸來吃，明年送你一個弟弟或妹妹」這種可以寫 |
| `hellish` | 地獄梗全開。黑色幽默、荒謬、反差、自虐式吐槽 |

**玩笑本身不違法,違法的是宣稱功效。** 「生蠔→弟弟妹妹」是文化梗、沒宣稱任何效果，可以寫；
「生蠔補腎壯陽」是宣稱，不行。界線在有沒有說「吃了會對身體怎樣」。

`hellish` 仍有硬線（這些不是法規問題，是**會讓帳號被出征**的東西）：
歧視、拿真實災難/疾病/死亡開玩笑、未成年性暗示、影射真人做人身攻擊、性騷擾式內容。
規則裡也教了地獄梗的正確用法——**把刀對準自己或抽象處境，不要對準特定的人或族群**。

🛡 紅隊也吃得到這個設定，並被明文要求「**不要把梗改掉**——你的工作是拆彈，不是消毒」。
...
| ✈️ `share` 分享型 | 衝分享數（唯一會帶出演算法之外的動作） | 冷知識／對號入座／懶人包／邀請函，讓人想傳給特定的人 |
| 😂 `story` 段子型 | 分享率最高，也最難寫 | 鋪陳 → 誤導 → **最後一句翻轉**；零品牌提及 |
...
**讀者輪廓**（`audienceInterests`）：`tags` 是「我賣什麼」，這欄是「**他們是誰**」。

只填 `tags` 的話，AI 會把每一則都硬轉回你的產品——段子型尤其致命，
結尾那句轉折就是別人看出「這是廣告」的地方，一看出來就沒人轉了。
填了這欄，貼文才寫得出「這群人會有反應的事」，而不只是「我想講的事」。

> 例：一家餐酒吧的讀者同時在追 EDM／DJ／K-POP／中西洋金曲、在吃美食、在看電競——
> 一則完全沒提到酒、但戳中他們的段子，照樣是這個帳號的段子。
...
待審核回覆的每張卡片都有 **🔄 重新生成**。第一版太 AI、太乖、不像這個帳號在講話時按它。

關鍵在**它不是單純重跑**——會把已經被你打槍的版本當成負面範例送進 prompt：

> 使用者按了「重新生成」，通常代表**太 AI、太乖、太無聊、不像這個帳號在講話**。
> **換一個完全不同的切入角度**，不是把同一句話換個說法。
> 可以換的方向：接不同的點、換情緒（吐槽↔共鳴↔自嘲）、換長度、改成反問、直接耍賤。
> 如果前面幾版都很客氣，這次就大膽一點。

所以連按會愈來愈有個性，而不是同一句話的三種寫法。最多記 3 版（prompt 不會無限膨脹）。

留言區的原貼文內容會存進 `posts.rootText`，重新生成時直接用，**不會再打一次 API**。
...
| `autoSchedule` | 產完稿要不要自動填建議時間，預設 `true`（**只填時間，仍要人核准**） |
| `dailyPublishCap` | 每個本地日最多排幾則，預設 15 |
| `minGapMinutes` | 兩則之間最少隔幾分鐘，預設 55 |
| `activeHours` | 只排在這個時段窗內，預設 `{"start":9,"end":25}`（25 = 隔日 01:00） |
| `bestTimeMinSamples` | 自家貼文少於幾則就不信實測時段、改用預設，預設 10 |
...
src/
  ...
  compliance.mjs     ⚖️ 法規紅線（食安法 §28 規則 + 關鍵字掃描）
  ...
  best_time.mjs      ⏰ 最佳發文時段（自家數據分桶；樣本不足回落並標明）
  autoschedule.mjs   ⏰ 建議發文時間（每日上限/最小間隔/活躍時段；只填時間不核准）
