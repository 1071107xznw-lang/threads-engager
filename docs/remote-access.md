...
**寫 `tailscale` 這個字，不要寫 IP。** 啟動時會自動找出當下的 Tailscale 位址
（CGNAT 100.64–127.x.x）——Tailscale 重新登入換發 IP 也不必改設定。

之後：**手機**用啟動訊息印出來的 `http://100.x.x.x:4321`，**這台機器**用
`http://localhost:4321`（綁單一介面時會額外多綁一個 127.0.0.1，兩邊都通）。

Tailscale 沒開的話會退回本機並在日誌說明，服務不會崩。

| | `HOST=0.0.0.0`（預設） | `HOST=tailscale` |
|---|---|---|
| 自己的手機（同 Wi-Fi） | ✅ | ✅ |
| 自己的手機（外面 / 4G） | ❌ | ✅ |
| 區網上的其他人 | ⚠️ 連得到 | ✅ 碰不到 |
