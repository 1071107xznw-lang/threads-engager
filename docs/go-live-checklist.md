...
- [ ] ⛔️ **企業驗證 / Business verification 通過**——**所有申請 Advanced Access 的 App 都要**。
      獨立於 App Review 的另一道流程，要公司登記文件，可能要幾個工作天。
      **沒過的話權限申請不會被處理**，先辦這個
- [ ] ⏱ **每個要送的權限，30 天內都成功呼叫過至少一次**——沒紀錄的會被直接退。
      把 dashboard 按一輪（產草稿 → 掃留言區 → 成效 → 核准送出一則）就全中
